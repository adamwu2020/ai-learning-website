...
// ── View switching ─────────────────────────────────────────
const viewTitles = { overview: ['Overview', 'Platform statistics'], users: ['Users', 'Manage all user accounts'] };
...
function renderUsersTable() {
  ...
  if (!users.length) { body.innerHTML = `<tr><td colspan="7" class="empty-msg">No users found</td></tr>`; return; }
...
function renderRecentUsers() {
  ...
  if (!recent.length) { body.innerHTML = `<tr><td colspan="4" class="empty-msg">No users yet</td></tr>`; return; }
...
async function openUserModal(id) {
  ...
  try {
    ...
    if (!transactions.length) {
      txBody.innerHTML = `<tr><td colspan="4" class="empty-msg">No transactions</td></tr>`;
    } else {
    ...
    document.getElementById('user-info-grid').innerHTML = `<div style="color:var(--danger)">${err.message}</div>`;
  }
}
...
function renderProgressTab(progress) {
  ...
  for (const [course, modules] of Object.entries(courseMap)) {
    ...
    const totalMap = NLP_MODULES;
    for (const [mod, lessons] of Object.entries(modules)) {
      const total = totalMap[mod] || lessons.length;
...
function subscriptionBadge(sub) {
  ...
  const active = parsed.expiresAt && new Date(parsed.expiresAt) > new Date();
  return active
